#!/bin/bash
# teardown.sh — Post-test cleanup for session-stop scenario.
#
# Sourced by ci-scripts/load-test.sh after the test completes.

PROJECT_NAME="${PROJECT_NAME:-session-stop}"
SESSION_CREATION_TIMEOUT="${SESSION_CREATION_TIMEOUT:-60}"
...
info "session-stop teardown complete"
