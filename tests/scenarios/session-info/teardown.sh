#!/bin/bash
# teardown.sh — Post-test cleanup for session-info scenario.
#
# Sourced by ci-scripts/load-test.sh after the test completes.

PROJECT_NAME="${PROJECT_NAME:-session-info}"
SESSION_CREATION_TIMEOUT="${SESSION_CREATION_TIMEOUT:-120}"
...
info "session-info teardown complete"
